...
$sheet->getStyle('B1:R1')->getFont()->setBold(true)->setSize(16);
$sheet->getStyle('B1:R1')->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);
$sheet->getStyle('A1:R1')->getFill()->setFillType(Fill::FILL_SOLID)->getStartColor()->setRGB('2563EB');
$sheet->getStyle('B1:R1')->getFont()->getColor()->setRGB('FFFFFF');
...
$headers = [
    ...
    'O' => 'Total Desc.',
    'P' => 'Salario Neto',
    'Q' => 'AFP Patronal (' . number_format($deductionRates['AFP']['employer_percentage'], 2) . '%)',
    'R' => 'SFS Patronal (' . number_format($deductionRates['SFS']['employer_percentage'], 2) . '%)'
];
...
foreach ($records as $record) {
    ...
    $sheet->setCellValue('N' . $row, $record['other_deductions']);
    $sheet->setCellValue('O' . $row, $record['total_deductions']);
    $sheet->setCellValue('P' . $row, $record['net_salary']);
    $sheet->setCellValue('Q' . $row, $record['afp_employer']);
    $sheet->setCellValue('R' . $row, $record['sfs_employer']);

    // Format currency
    foreach (['F', 'H', 'I', 'J', 'K', 'L', 'M', 'N', 'O', 'P', 'Q', 'R'] as $col) {
...
$sheet->setCellValue('N' . $row, $totals['other']);
$sheet->setCellValue('O' . $row, $totals['deductions']);
$sheet->setCellValue('P' . $row, $totals['net']);
$sheet->setCellValue('Q' . $row, $totals['afp_pat']);
$sheet->setCellValue('R' . $row, $totals['sfs_pat']);

foreach (['H', 'I', 'J', 'K', 'L', 'M', 'N', 'O', 'P', 'Q', 'R'] as $col) {
    $sheet->getStyle($col . $row)->getNumberFormat()->setFormatCode('"RD$"#,##0.00');
}
...
$sheet->getStyle('A' . $row . ':R' . $row)->applyFromArray($totalsStyle);
