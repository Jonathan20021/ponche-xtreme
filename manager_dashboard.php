...
function stateEscapeHtml(value) {
...
function updateStateHistory(timeline, stateTotals, dayTotals) {
    ...
    if (!Array.isArray(timeline) || timeline.length === 0) {
        ...
        box.innerHTML = `
            <div class="text-center text-muted py-4">
                <p class="text-sm">Sin disposiciones registradas hoy</p>
            </div>
        `;
        return;
    }
    ...
    bar.innerHTML = timeline.map(seg => {
        ...
        return `<span class="state-bar-seg" style="width:${pct}%;background:${seg.color}" title="${stateEscapeHtml(tip)}"></span>`;
    }).join('');
    ...
    const totalsHtml = (Array.isArray(stateTotals) ? stateTotals : []).map(s => `
        <div class="state-total-row">
            ...
            <span class="state-total-label">
                <i class="${s.icon}"></i> ${stateEscapeHtml(s.label)}
            </span>
            <span class="state-total-episodes">${s.episodes}x</span>
            <span class="state-total-time">${stateEscapeHtml(s.duration_formatted)}</span>
    ...
    const segmentsHtml = timeline.map(seg => `
        <div class="state-seg-row${seg.is_open ? ' is-open' : ''}">
            <span class="state-seg-time">${stateEscapeHtml(seg.start)} → ${stateEscapeHtml(seg.end || 'ahora')}</span>
            <span class="state-seg-label">
                <span class="state-dot" style="background:${seg.color}"></span>
                <i class="${seg.icon}"></i> ${stateEscapeHtml(seg.label)}
            </span>
            <span class="state-seg-duration">${stateEscapeHtml(seg.duration_formatted)}</span>
