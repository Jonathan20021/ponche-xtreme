<?php
/**
 * Manual Recruitment Report Sender
 *
 * Modes:
...
if (function_exists('userHasPermission')) {
    try {
        $hasAccess = userHasPermission('settings') || userHasPermission('hr_report') || userHasPermission('hr_recruitment');
    } catch (Throwable $e) {
        $hasAccess = false;
    }
}
if (!$hasAccess) {
    $allowedRoles = ['ADMIN', 'ADMINISTRATOR', 'DESARROLLADOR', 'IT', 'HR', 'RECURSOS HUMANOS', 'DIRECTOR', 'GENERALMANAGER', 'OPERATIONSMANAGER', 'GERENTEDEOPERACIONES', 'ENCARGADODEGESTIONHUMANA', 'SUPERVISOR'];
...
try {
    ...
    try {
        ...
        if (function_exists('log_custom_action')) {
            log_custom_action(
                ...
                'reports',
                'send',
                'Reporte diario de reclutamiento enviado manualmente',
                'recruitment_report',
                null,
                [
                    'mode'             => $mode,
        ...
        error_log('send_recruitment_report log error: ' . $e->getMessage());
    }
    ...
    error_log('send_recruitment_report error: ' . $e->getMessage());
