<?php
/**
 * Manual GHL Voice AI Executive Report Sender
 *
 * Modes:
...
if (function_exists('userHasPermission')) {
    try {
        $hasAccess = userHasPermission('settings') || userHasPermission('voice_ai_reports');
    } catch (Throwable $e) {
        $hasAccess = false;
    }
}
...
try {
    ...
    if ($mode === 'send_preview') {
        ...
        if (empty($recipients)) {
            echo json_encode([
                'success' => false,
                'error'   => 'No hay destinatarios configurados. Agrega correos en el campo "Destinatarios" y haz clic en "Guardar Configuración" antes de enviar.',
            ]);
            exit;
        }
    }
    ...
    if (empty($reportData['available'])) {
        echo json_encode([
            'success' => false,
            'error'   => 'No se pudo generar el reporte: ' . ($reportData['error'] ?? 'datos no disponibles'),
        ]);
        exit;
    }
    ...
    try {
        ...
        if (function_exists('log_custom_action')) {
            log_custom_action(
                ...
                'reports',
                'send',
                'Reporte ejecutivo GHL Voice AI enviado manualmente',
                'ghl_report',
                null,
                [
                    'mode'             => $mode,
        ...
        error_log('send_ghl_report log error: ' . $e->getMessage());
    }
    ...
    echo json_encode([
        ...
        'data'    => [
            ...
            'date'                => $reportData['date'],
            'integration'         => $reportData['integration'],
            'integrations_count'  => $reportData['integrations_count'] ?? 1,
            'totals'              => $reportData['totals'],
            'ai_generated'        => $aiSummary !== '',
        ],
    ]);
    ...
    error_log('send_ghl_report error: ' . $e->getMessage());
