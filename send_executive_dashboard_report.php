<?php
/**
 * Manual Executive Dashboard Summary Sender ("Cierre del día")
 *
 * Modes:
...
if (function_exists('userHasPermission')) {
    try {
        $hasAccess = userHasPermission('settings') || userHasPermission('executive_dashboard');
    } catch (Throwable $e) {
        $hasAccess = false;
    }
}
if (!$hasAccess) {
    $allowedRoles = ['ADMIN', 'ADMINISTRATOR', 'DESARROLLADOR', 'IT', 'DIRECTOR', 'GENERALMANAGER', 'OPERATIONSMANAGER', 'GERENTEDEOPERACIONES'];
...
if (!$hasAccess) {
    ...
    echo json_encode([
        'success' => false,
        'error'   => 'No tienes permisos para esta acción (requiere rol ejecutivo o de Administrador)',
...
try {
    ...
    try {
        ...
        if (function_exists('log_custom_action')) {
            log_custom_action(
                ...
                'reports',
                'send',
                'Cierre ejecutivo del día enviado manualmente',
        ...
        error_log('send_executive_dashboard_report log error: ' . $e->getMessage());
    }

    echo json_encode([
        'success' => true,
        'message' => 'Cierre ejecutivo enviado a ' . count($recipients) . ' destinatario(s).',
        'data'    => [
            'recipients_count' => count($recipients),
            'date'             => $reportData['date'],
            'totals'           => $reportData['totals'] ?? [],
    ...
    error_log('send_executive_dashboard_report error: ' . $e->getMessage());
