<?php
/**
 * Manual Inventory Report Sender
 *
 * Modes:
...
if (function_exists('userHasPermission')) {
    try {
        $hasAccess = userHasPermission('settings') || userHasPermission('hr_report') || userHasPermission('inventory');
    } catch (Throwable $e) {
        $hasAccess = false;
    }
}
...
try {
    ...
    try {
        ...
        if (function_exists('log_custom_action')) {
            log_custom_action(
                ...
                'reports',
                'send',
                'Reporte diario de inventario enviado manualmente',
                'inventory_report',
                null,
                [
                    'mode'             => $mode,
        ...
        error_log('send_inventory_report log error: ' . $e->getMessage());
    }
    ...
    error_log('send_inventory_report error: ' . $e->getMessage());
